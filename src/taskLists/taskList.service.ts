...
export class TaskListService {
  ...
  constructor() {
    ...
    this.userPermissionRepository = AppDataSource.getRepository(UserPermissions);
  }
  async create(title: string, user: User): Promise<TaskList> {
    try {  
      ...
      const newPermissions = this.userPermissionRepository.create(userPermissionsData)
  ...
  async delete(taskList:TaskList): Promise<DeleteResult> {
    try {
      const deleteTask = await this.taskListRepository.delete(taskList);
      return deleteTask;
    } catch (err) {
      console.log(err);
    }
